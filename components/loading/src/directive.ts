...
// 获取属性
function getConfig(el:any, binding:any) {
  const text = attrs(el, 'z-loading-text');
  const icon = attrs(el, 'z-loading-icon');
  const textColor = attrs(el, 'z-loading-text-color');
  const iconColor = attrs(el, 'z-loading-icon-color');
  const background = attrs(el, 'z-loading-background');
...
// 插入节点
function append(el:any, binding:any) {
...
// 移除节点
function remove(el:any, binding:any) {
  // 移除动态创建的 div 节点
  if (binding.modifiers.screen) {
